...
vector<Point> convexcut(vector<Point> &p,Segment l) {
	...
	for(int i=0;i<n;i++) {
		...
		if(ccw(l.p1,l.p2,p[i])*ccw(l.p1,l.p2,p[(i+1)%n]) < 0.0) {
			if(!(l.toVec()).is_parallel(p[i]-p[(i+1)%n])) {
				q.push_back(intersection(l,Segment(p[i],p[(i+1)%n])));
			}
		}
	}
	return q;
...
int main(void){
	...
	REP(i,q) {
		cin >> x1 >> y1 >> x2 >> y2;
		Segment t(x1,y1,x2,y2);
		cout << fixed << ROUND(10) << getArea(convexcut(p,t)) << endl;
